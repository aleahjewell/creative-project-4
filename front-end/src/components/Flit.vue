...
export default {
  ...
  computed: {
    ...
    profilePic() {
      return this.user ? this.user.profilePicPath : "";
    },
    date() {
    ...
    image() {
      return this.flit.imagePath;
    },
  },
  methods: {
    async post() {
      try {
        console.log(this.flit);
        let url = "/api/users/" + this.flit.user.id + "/flits";
        console.log(url);
        await axios.post(url, {
          flit: this.textInput,
          date: this.flit.date,
          imagePath: this.imageInput,
        });
